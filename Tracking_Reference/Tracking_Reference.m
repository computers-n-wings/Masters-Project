function [problem,guess] = Tracking_Reference
% Tracking_Reference: Determine the optimal parameters 
%
...
% Global parameters
[ Kf, mh, mw, mf, mb, Lh, La, Lw, grav, F_v1, F_c1, ta] = setup_heli_3d_configuration();
...
problem.states.xl = [-epsmax -rhomax -Inf -epsdotmax -rhodotmax -lamdotmax -Inf -Inf];
problem.states.xu = [epsmax rhomax Inf epsdotmax rhodotmax lamdotmax Inf Inf];

% Terminal state bounds. xfl=< xf <=xfu

problem.states.xfl = [-epsmax -rhomax -Inf -epsdotmax -rhodotmax -lamdotmax -Vmax -Vmax];
problem.states.xfu = [epsmax rhomax Inf epsdotmax rhodotmax lamdotmax Vmax Vmax];
...
% rho = 10;
% p1=p(:,1); p2=p(:,2);
% penalty = rho*(ones(1,length(p))*p1 + ones(1,length(p))*p2);

%% Stage Cost

stageCost = (x(:,1)-xr(:,1)).^2 + (x(:,2)-xr(:,2)).^2 + (x(:,3)-xr(:,3)).^2 + penalty;
%------------- END OF CODE --------------
...
F_v1 = data.F_v1; F_c1 = data.F_c1; ta = data.ta;
...
% Linear Frictionless model with Actuator Lag
c1 = La*Kf/(2*mf*(La^2)+mw*(Lw^2));
c2 = 0.5*Kf/(mf*Lh);
c3 = -(Lw*mw-2*La*mf)*grav/(mw*(Lw^2)+2*mf*(Lh^2)+2*mf*(La^2));
dx(:,1) = x4;
dx(:,2) = x5;
dx(:,3) = x6;
dx(:,4) = c1*x7+c1*x8;
